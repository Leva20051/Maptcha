...
if (!/^[A-Za-z0-9_]+$/.test(config.database)) {
  throw new Error(`Unsafe database name "${config.database}". Use only letters, numbers, and underscores.`);
}
...
async function main() {
  ...
  const users = [
    { id: 1, username: "Praveen", email: "praveen@ucalgary.ca", password: "CafeAdmin2026!", regDate: "2025-03-15" },
    { id: 2, username: "saira", email: "saira@ucalgary.ca", password: "CuratorSaira2026!", regDate: "2025-03-16" },
    { id: 3, username: "leo", email: "leo@ucalgary.ca", password: "CuratorLeo2026!", regDate: "2025-01-17" },
    { id: 4, username: "personA", email: "personA@gmail.com", password: "UserA2026!", regDate: "2026-03-15" },
    { id: 5, username: "personB", email: "personB@gmail.com", password: "UserB2026!", regDate: "2026-02-02" },
    { id: 6, username: "mia", email: "mia@ucalgary.ca", password: "UserMia2026!", regDate: "2026-03-22" },
  ];

  for (const user of users) {
    await connection.query(
      "INSERT INTO `User` (`UserID`, `Username`, `Email`, `Password`, `RegDate`) VALUES (?, ?, ?, ?, ?)",
      [user.id, user.username, user.email, hashPassword(user.password), user.regDate],
    );
  }
  ...
  console.log(`Bootstrapped database "${config.database}" on ${config.host}:${config.port}`);
  console.log("Demo credentials:");
  console.log("  Admin   -> Praveen / CafeAdmin2026!");
  console.log("  Curator -> saira / CuratorSaira2026!");
  console.log("  Curator -> leo / CuratorLeo2026!");
  console.log("  User    -> personA / UserA2026!");
  console.log("  User    -> personB / UserB2026!");
  console.log("  User    -> mia / UserMia2026!");
